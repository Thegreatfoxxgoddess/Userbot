...
async def startmute(event):
    ...
    if any([x in event.raw_text for x in ("/mute", "!mute")]):
        ...
        if event.pattern_match.group(1) is not None:
            ...
            return await event.edit("Please reply to a user or add their user id into the command to mute them.")
        ...
        if "admin_rights" in vars(chat) and vars(chat)["admin_rights"] is not None: 
            if chat.admin_rights.delete_messages is True:
                ...
                return await event.edit("`You're Missing Admin Rights`")
        ...
        if is_muted(userid, chat_id):
            return await event.edit("`User Is Already Muted!`")
        try:
            mute(userid, chat_id)
        except Exception as e:
            await event.edit("Error occured!\nError is " + str(e))
        else:
            await event.edit("`User Muted Successfully!`\nMessage [Max](https://t.me/maxpayne7000) To Get Yourself Unmuted.")
...
async def endmute(event):
    ...
    if any([x in event.raw_text for x in ("/unmute", "!unmute")]):
        ...
        if event.pattern_match.group(1) is not None:
            ...
            return await event.edit("Please reply to a user or add their user id into the command to unmute them.")
        chat_id = event.chat_id
        if not is_muted(userid, chat_id):
            return await event.edit("`User Can Speak Freely Already!`")
        try:
...
async def startmute(event):
    ...
    if any([x in event.raw_text for x in ("/mute", "!mute")]):
        ...
        try:
            ...
            await event.edit("`User Muted Successfully!`\nMessage Max To Get Yourself Unmuted.")
...
async def hehehe(event):
    ...
    if event.is_private:
        if not pmpermit_sql.is_approved(chat.id):
            pmpermit_sql.approve(chat.id, "Owner Of The Repo!")
            await borg.send_message(chat, "`OMG Max Is Here!`")
